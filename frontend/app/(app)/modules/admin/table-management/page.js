...
export default function TableManagement() {
    ...
    const handleDrop = async (e) => {
        ...
        // Calculate percentages based on the container rect
        const containerRect = e.currentTarget.getBoundingClientRect();
        let xPos = ((e.clientX - containerRect.left) / containerRect.width) * 100;
        let yPos = ((e.clientY - containerRect.top) / containerRect.height) * 100;

        // Keep it bounded (assuming table width is represented roughly by percentage)
        xPos = Math.max(0, Math.min(xPos, 90));
        yPos = Math.max(0, Math.min(yPos, 90));

        // Optimistically update tables locally
        setTables((prev) =>
            prev.map((t) =>
                t._id === draggedTable._id
                    ? { ...t, positionX: xPos, positionY: yPos }
                    : t
            )
        );
        ...
        try {
            const token = localStorage.getItem("token");
            await fetch(`${API_URL}/tables/${draggedTable._id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
                },
                body: JSON.stringify({ positionX: xPos, positionY: yPos }),
            });
        } catch (err) {
    ...
    const handleCheckAvailability = async () => {
        if (!resForm.date || !resForm.time || !resForm.endTime || !resForm.guestCount) {
            showMessage("Missing Information", "Please fill in date, times and guest count to check availability.", "info");
            return;
        }
        if (resForm.time >= resForm.endTime) {
            showMessage("Invalid Times", "End time must be after start time.", "error");
            return;
        }
        ...
        try {
            ...
            const endAt = new Date(`${resForm.date}T${resForm.endTime}`).toISOString();
    ...
    const handleEditReservation = (reservation) => {
        ...
        setResForm({
            ...
            endTime: new Date(reservation.endAt).toLocaleTimeString([], {
                hour12: false,
                hour: "2-digit",
                minute: "2-digit",
            }),
    ...
    const downloadQRCode = () => {
        ...
        const targetUrl = `${window.location.origin}/?table=${selectedQRTable._id}`;
        const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=${encodeURIComponent(targetUrl)}`;
        
        // Fetch the image as a blob so we can force a download via anchor tag
        fetch(qrUrl)
            .then(res => res.blob())
            .then(blob => {
                ...
                a.href = url;
                a.download = `Table-${selectedQRTable.tableNumber}-QRCode.png`;
    ...
    return (
        <div className="flex-1 p-8 pt-24 min-h-screen relative bg-[#181E38]">
            ...
            <div className="relative">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                    <div className="flex items-center bg-white/10 backdrop-blur-sm p-1 rounded-xl shadow-sm border border-white/10">
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${viewMode === "FLOOR_PLAN"
                                ? "bg-[#005477] text-white shadow-md"
                                : "text-slate-300 hover:bg-white/10"
                                }`}
                        >
                ...
                {/* Main Content */}
                {viewMode === "FLOOR_PLAN" && (
                    <Card className="bg-[#1A1F36] border border-white/5 shadow-2xl rounded-[2rem] overflow-hidden animate-in fade-in h-[600px] relative mt-4">
                        <div className="p-4 border-b border-white/10 bg-[#232943] flex items-center justify-between">
                            <div>
                                <h2 className="text-xl font-semibold text-white">Visual Floor Plan</h2>
                                <p className="text-xs text-slate-400 mt-1">Drag and drop tables to adjust the restaurant floor layout.</p>
                            </div>
                            <div className="flex items-center gap-4 text-xs font-bold uppercase tracking-widest text-slate-300">
                                ...
                                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-amber-500" /> Occupied</div>
                                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-slate-500" /> Maintenance</div>
                            </div>
                        </div>
                        <div className="absolute inset-x-4 inset-y-[88px] bottom-4">
                            <div
                                className="w-full h-full border-2 border-dashed border-white/10 rounded-2xl relative overflow-hidden bg-[#181E38]"
                                style={{
                            ...
                            >
                                {tables.map(table => (
                                    <div
                                        ...
                                        className={cn(
                                            "absolute cursor-grab active:cursor-grabbing transform -translate-x-1/2 -translate-y-1/2 rounded-full p-2 flex flex-col items-center justify-center gap-1 shadow-2xl transition-transform hover:scale-105 border-[3px] w-24 h-24",
                                            table.status === 'AVAILABLE' ? "bg-[#181E38] border-green-500 shadow-green-500/20" :
                                                table.status === 'OCCUPIED' ? "bg-[#181E38] border-amber-500 shadow-amber-500/20" :
                                                    "bg-[#181E38] border-slate-500 opacity-60"
                                        )}
                                        style={{
                                            left: `${table.positionX || Math.floor(Math.random() * 80) + 10}%`,
                                            top: `${table.positionY || Math.floor(Math.random() * 80) + 10}%`
                                        }}
                                    >
                                        <span className={cn("text-xl font-black", table.status === 'AVAILABLE' ? "text-green-500" : table.status === 'OCCUPIED' ? "text-amber-500" : "text-white")}>{table.tableNumber}</span>
                                                    ...
                                                    <td className="px-8 py-5 text-right">
                                                        <div className="flex justify-end items-center gap-3">
                                                            ...
                                                            <button
                                                                ...
                                                                className="p-2.5 rounded-xl bg-[#408c8c]/10 hover:bg-[#408c8c] text-[#408c8c] hover:text-white transition-all hover:scale-110 shadow-sm border border-[#408c8c]/20 group/btn"
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <Input
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        setAvailableTables([]);
                                                        if (resErrors.endTime || resErrors.tableId) {
                                                            setResErrors((prev) => ({ ...prev, endTime: null, tableId: null }));
                                                        }
                                                    }}
                                                />
                                                {resErrors.endTime && (
                                                    <p className="text-[10px] text-red-500 mt-1 font-bold">{resErrors.endTime}</p>
                                                )}
                                            </div>
                                        </div>
                ...
                {showQRModal && selectedQRTable && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                        <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl animate-in fade-in zoom-in duration-200 text-center relative overflow-hidden">
                            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
                                <h2 className="text-xl font-bold text-gray-900">Table {selectedQRTable.tableNumber} QR</h2>
                                <button
                            ...
                            <div className="p-8 flex flex-col items-center justify-center">
                                <div className="bg-white p-4 rounded-xl shadow-[0_0_15px_rgba(0,0,0,0.1)] mb-6">
                                    <img 
                                        src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(window.location.origin + "/?table=" + selectedQRTable._id)}`}
                                        alt={`QR Code for Table ${selectedQRTable.tableNumber}`}
                                        className="w-48 h-48"
                                    />
                                </div>
                                <p className="text-sm text-slate-500 mb-6 px-4">
                                    Place this QR code on Table {selectedQRTable.tableNumber}. Scanning it will pre-assign this table for guest orders.
                                </p>
                                <Button
                                    onClick={downloadQRCode}
                                    className="w-full bg-[#408c8c] hover:bg-[#408c8c]/90 text-white font-bold h-11 flex items-center justify-center gap-2"
                                >
